...
const AudienceAnalytics: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <Card className="flex justify-center items-center p-8 h-48">
        <Spinner />
      </Card>
    );
  }
  ...
  if ((!data?.gender || data.gender.length === 0) && (!data?.country || data.country.length === 0)) {
    return (
      <Card className="p-6">
        <p className="text-center text-gray-400">No audience demographic data available yet.</p>
      </Card>
    );
  }
